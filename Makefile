.PHONY: help tidy seed run test test-race test-integration test-all test-coverage simple-coverage docker-up docker-down fmt lint check check-it clean
...
help:
	...
	@echo "  make check-it        - Run fmt + lint + test-all (full validation and ITs, requires Docker!)"
...
check: fmt lint test
	@echo "All checks (besides ITs) passed :D (formatter + linter + test)"

check-it: check test-integration
	@echo "All checks with ITs passed :D (formatter + linter + test + ITs)"
